...
public:
  ...
  MyApplication( int argc, char* argv[] ) : 
    ...
    labels_( 0 ), canvas_( NULL ), frame_( NULL ), vtg_( 0 ), lg_( NULL ), 
    ...
    vevo_( 0 ), vxmls_( 0 ), vname_( 0 )
  {
    ...
    try {
      ...
      ymaxmin_[ 0 ] = args.get( "ymax", 0.0 );
      ymaxmin_[ 1 ] = args.get( "ymin", 0.0 );
      xmaxmin_[ 0 ] = args.get( "xmax", 1.0 );
      xmaxmin_[ 1 ] = args.get( "xmin", 0.01 );

      labels_.resize( 2, "" );
      labels_[ 0 ] = args.get( "xtitle", "" );
      labels_[ 1 ] = args.get( "ytitle", "" );

      canvas_ = new TCanvas( "c" );
      if( args.hasOpt( "logx" ) ) canvas_->SetLogx();
      if( args.hasOpt( "logy" ) ) canvas_->SetLogy();

      double ymarg_ = args.get( "fymargin", 0.1 ) * ( ymaxmin_[ 0 ] - ymaxmin_[ 1 ] );
      frame_ = canvas_->DrawFrame( xmaxmin_[ 1 ], ymaxmin_[ 1 ] - ymarg_,
				   xmaxmin_[ 0 ], ymaxmin_[ 0 ] + ymarg_ );
...
private:
  ...
  void draw( const double& q2, const int& iflav ){
    ...
    // draw frame again to reflect the possible yparam change
    double ymarg_ = args.get( "fymargin", 0.1 ) * ( ymaxmin_[ 0 ] - ymaxmin_[ 1 ] );
    frame_ = canvas_->DrawFrame( xmaxmin_[ 1 ], ymaxmin_[ 1 ] - ymarg_,
				 xmaxmin_[ 0 ], ymaxmin_[ 0 ] + ymarg_ );

    if( labels_.size() == 2 ){
      TAxis* tax = frame_->GetXaxis();
      tax->SetTitle( args.hasOpt( "xtitle" ) ? labels_[ 0 ].c_str() :
		     xval_.c_str() );
      tax->SetTitleSize( 0.052 );      
      tax->SetTitleOffset( 0.50 );
      tax->SetLabelSize( 0.042 );
      tax->SetLabelOffset( 0.01 );
      TAxis* tay = frame_->GetYaxis();
      tay->SetTitle( args.hasOpt( "ytitle" ) ? labels_[ 1 ].c_str() :
		     this->get_header( q2, iflav ).c_str() );
      tay->SetTitleSize( 0.048 );
      tay->SetTitleOffset( 1.0 );
    ...
    /*
    TGaxis tgax;
    tgax.ImportAxisAttributes( frame_->GetXaxis() );
    tgax.Draw();
    */
    /*
    TAxis* tx = frame_->GetXaxis();
    TAxis taxx( tx->GetNbins(), tx->GetXmin(), tx->GetXmax() );
    taxx.Draw();
    */
    /*
    frame_->Draw( "same" );
    ...
    */
    /*
    frame_->GetXaxis()->Draw( "same" );
    frame_->GetYaxis()->Draw( "same" );
    */
    /*
    frame_->SetFillStyle( 0 );
    frame_->SetFillColor( 0 );
    frame_->Draw( "same" );
    frame_ = canvas_->DrawFrame( xmaxmin_[ 1 ], ymaxmin_[ 1 ] - ymarg_, 
				 xmaxmin_[ 0 ], ymaxmin_[ 0 ] + ymarg_ );
    */
  ...
  vector< TGraph* > getTGraphs( const double& q2, const int& iflav, 
    ...
    */
	...
	if( ! setvfile( vx_base[ i ], q2, iflav, vname_[ i_werr ], 
      ...
      if( ! args.hasOpt( "ymax" ) && tmpy_[ 0 ] > ymaxmin_[ 0 ] )
	ymaxmin_[ 0 ] = tmpy_[ 0 ]; // dynamical ymax
      if( ! args.hasOpt( "ymin" ) && tmpy_[ 0 ] < ymaxmin_[ 1 ] )
	ymaxmin_[ 1 ] = tmpy_[ 0 ]; // dynamical ymin
    }

    tgv_.push_back( new TGraph( vx_base.size(), 
				&*vx_base.begin(), &*vy_base[ 0 ].begin() ) );
    ...
    if( args.hasOpt( "fill" ) ){
      ...
      for( int i = 0; i < vx_base.size(); i++ ){
	tmpx_[ i ] = vx_base[ i ];
	tmpy_[ i ] = vy_base[ 0 ][ i ] + vy_base[ 1 ][ i ];
      }
      for( int i = vx_base.size() - 1; i > -1; i-- ){
	tmpx_[ 2 * vx_base.size() - i - 1 ] = vx_base[ i ];
	tmpy_[ 2 * vx_base.size() - i - 1 ] = vy_base[ 0 ][ i ] - vy_base[ 1 ][ i ];
      }
      tgv_.push_back( new TGraph( tmpx_.size(), 
				  &*tmpx_.begin(), &*tmpy_.begin() ) );
      vtag_.push_back( "ERROR_FILL" ); vtag = vtag_; // copy to vtag again

      double tole_( args.get( "tolerance", 1.0 ) );
      for( int i = 0; i < vx_base.size(); i++ ){
	tmpx_[ i ] = vx_base[ i ];
	tmpy_[ i ] = vy_base[ 0 ][ i ] + tole_ * vy_base[ 1 ][ i ];
      }
      for( int i = vx_base.size() - 1; i > -1; i-- ){
	tmpx_[ 2 * vx_base.size() - i - 1 ] = vx_base[ i ];
	tmpy_[ 2 * vx_base.size() - i - 1 ] = vy_base[ 0 ][ i ] - tole_ * vy_base[ 1 ][ i ];
      }
      tgv_.push_back( new TGraph( tmpx_.size(), 
				  &*tmpx_.begin(), &*tmpy_.begin() ) );
      vtag_.push_back( "TOL_FILL" ); vtag = vtag_; // copy to vtag again
    }
    ...
    // set line attributes
    for( int i = 0; i < tgv_.size(); i++ ){
      ostringstream ost; 

      ost << "setLC" << i + 1;
      tgv_[ i ]->SetLineColor( args.get( ost.str(), 1 ) );
      ost.str( "" ); ost.clear();
      ost << "setLS" << i + 1;
      tgv_[ i ]->SetLineStyle( args.get( ost.str(), 1 ) );
      ost.str( "" ); ost.clear();
      ost << "setLW" << i + 1;
      tgv_[ i ]->SetLineWidth( args.get( ost.str(), 1 ) );
      ost.str( "" ); ost.clear();

      ost << "setFC" << i + 1;
      tgv_[ i ]->SetFillColor( args.get( ost.str(), 1 ) );
      ost.str( "" ); ost.clear();
      ost << "setFS" << i + 1;
      tgv_[ i ]->SetFillStyle( args.get( ost.str(), 3002 ) );
      ost.str( "" ); ost.clear();
    }
  ...
  void draw_boundary(){
    ...
    if( args.hasOpt( "zerohorizon" ) ){
      line_.SetLineColor( 1 );
      line_.SetLineStyle( 1 );
      line_.SetLineWidth( 1 );
      line_.DrawLine( xmaxmin_[ 1 ], 0.0, xmaxmin_[ 0 ], 0.0 );
    }
