...
#include <IntegSPolVBoson/StrFunc.hh>
#include <IntegSPolVBoson/nStrFunc.hh>
...
int main( int argc, char* argv[] ) {
  ...
  try {
    ...
    IntegSPolVBoson::StrFunc dcs( args, IntegXsec::Charge::p, 
    ...
    IntegSPolVBoson::nStrFunc dcsnx( args, IntegXsec::Charge::p,
				      QCD::Flavor::PRp );
    ...
    */
    ...
    // For Error Calc
    ErrorCalc::ErrCalc eCalcx( &dcs );
    ErrorCalc::ErrCalc eCalcn( &xdcsnx );
	...
	for( int iq2 = 0; iq2 < q2.size(); iq2++ ){
	  ...
	  if( args.hasOpt( "wError" ) ){
	    ...
	    for( int i = 0; i < x.size(); i++ ){
	      ...
	      double x02 = sqrt( tau ) * exp( - x[ i ] );
	      // d sig / d y //
	      cout << setw( 13 ) <<  sqrt( eCalcx( x01, x02 ) ) 
		/  S_ /  x01 / x02 * GBscale << flush;
	    }
	    timer.stop();
	    cout << setw( 13 ) << timer.check( Timer::Sec ) << " sec" << flush;
	...
	if( args.hasOpt( "wError" ) ){
	  ...
	  for( int i = 0; i < x.size(); i++ ){
	    ...
	    cout << setw( 13 ) 
		 << sqrt( eCalcn( unitv, unitv )  ) / S_ * GBscale 
		 << flush;
	  }
	  timer.stop();
	  cout << setw( 13 ) << timer.check( Timer::Sec ) << " sec" << flush;
