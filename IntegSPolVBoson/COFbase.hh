#ifndef _IntegSPolVBoson_COFbase_hh_
#define _IntegSPolVBoson_COFbase_hh_
...
namespace IntegSPolVBoson {
  ...
  public:
    ...
    // single VBson production treats polPDF and unpolPDF at the same time
    // rescale function should be default ( IntegXsec/ConvIntegrand )
  ...
  protected:
    ...
    Transform::RealCompFunction* dUGC_;
...
template< class DD >
DD DUGC::SWC( const DD& x, const DD& z ){
  // d Dsig / dt dy
  /*
  return 2.0 * z * ( pow( x * z , 2 ) - pow( x * z - 1.0 , 2 ) ) 
    * ( x * z + 1.0 ) / ( x + z ) / ( 1.0 + z );
...
#endif //  _IntegSPolVBoson_COFbase_hh_
