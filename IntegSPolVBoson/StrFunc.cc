...
double StrFunc::charge( const int& i, const int& j ){
  ...
  IntegSPolVBoson::CoeffFunc* ck_ 
    = dynamic_cast< IntegSPolVBoson::CoeffFunc* >( this->xsecInteg()
						    ->coeff() );
  double tmpchr;
  if( !( ck_->getwjudge() ) ){
    // Z process
    tmpchr = ck_->vchar()[ ( j != 0 ? j + 6 : i + 6 ) ];
  } else {
    // W process
    if( i * j != 0 ){
      tmpchr = ck_->vvchar()[ i + 6 ][ j + 6 ];
    } else {
      tmpchr = ck_->sumvvchar( j != 0 ? j : i );
    }
  }

  tmpchr *= ck_->oafactor( i, j );
