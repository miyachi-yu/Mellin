  ...
  Xsection::XsecComp()
{
  ...
  try {
    ...
    this->insert( unpolPDF::Evo::instance(), h );
    dynamic_cast< unpolPDF::Evo* >( this->vevo()[ 1 ] )->constNf( false );
...
nXsecComp::nXsecComp( Evolution::KernelBase *pdf1, 
  ...
  Xsection::XsecComp()
{
  ...
  // prepare 1st Evolution kernel for polVBoson function ( n )
  if( dynamic_cast< polPDF::Evo* >( pdf1 ) ){
    this->insert( pdf1, Flavor::PRp );
  } else {
    ARG_ERR << __FILE__ << ":" << __LINE__ 
	    << "\tpdf1 should be either polPDF::Evo"
	    << endl;
    throw( 1 );
  }
  
  // prepare 2nd Evolution kernel for polVBoson function ( m )
  if( dynamic_cast< unpolPDF::Evo* >( pdf2 ) ||
      dynamic_cast< unpolPDF::CTEQ6pdf* >( pdf2 ) ){
    this->insert( pdf2, h );
  } else {
    ARG_ERR << __FILE__ << ":" << __LINE__ 
            << "\tpdf2 should be either unpolPDF::Evo or unpolPDF::CTEQ6pdf"
            << endl;
    throw( 1 );
  }
...
void nXsecComp::coefficients( const IntegXsec::Charge::TARGET& n ){
  ...
  if( ! args.hasOpt( "PInteg" ) ){
    if( args.hasOpt( "ShortInteg" ) ){ 
      coeff = new IntegSPolVBoson::StrFunc( this, args, n, 4, 0, 1.0E-2 );
    } else {
      coeff = new IntegSPolVBoson::StrFunc( this, args, n, 4, 6, 1.0E-4 );
    }
  } else {
    /*
    if( args.hasOpt( "ShortInteg" ) ){ 
      coeff = new IntegSPolVBoson::PStrFunc( this, args, n, 4, 0, 1.0E-2 );
    } else {
      coeff = new IntegSPolVBoson::PStrFunc( this, args, n, 4, 6, 1.0E-4 );
    }
    */
    throw( 1 );
  }
  ...
  this->vevo()[ 1 ]->alpha().setMURfact( rescaleQ2 / rescaleUPDF );
  
  RealFunction* rf_1_ = ( rescalePPDF != 1.0 ? 
			  new Xsection::XsecComp::SimpleRescale( rescalePPDF )
			  : ( RealFunction* ) NULL );
  RealFunction* rf_2_ = ( rescaleUPDF != 1.0 ? 
