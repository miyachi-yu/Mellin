...
void CoeffFunc::update(){
  ...
  for( int i = - nf; i <= nf; i++ ){
	...
	*/
	...
	cc *= this->oafactor( i, j );
	//	cout << i << " " << j << " " << oafactor( i, j ) << endl;
...
double CoeffFunc::oafactor( const int& i, const int& j ){
  // ref hep-ph/9710508 (Gehrmann)
  if( i * j != 0 ){
    // assume evo[ 0 ] polarized PDF ( i ) & evo[ 1 ] unpolarized PDF ( j )
    if( i > 0 ) return -1.0;
  } else {
    if( ( i != 0 ? i : j ) < 0 ) return -1.0;
  }
  return 1.0;
}
