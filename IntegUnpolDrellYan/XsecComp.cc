  ...
  IntegXsec::XsecIntegrand()
{
  ...
  try {
    ...
    // prepare Evolution kernel for unpolDrellYan function
    unpolPDF::Evo *pdf = unpolPDF::Evo::instance();
    pdf->constNf( false );
    //1st PDFs
    this->insert( pdf, QCD::Flavor::PRp );
    ...
    coeff->constNf( false );
    this->coeff( coeff );
    dynamic_cast< IntegXsec::CKernel* >( this->coeff() )->wilsons( new WC( this ) );
