...
int main( int argc, char* argv[] ){
  ...
  try{
    ...
    IntegPolSIDIS::CoeffFunc* kernel_;
...
*/
    ...
    for( int k = 0; k < 1; k++ ){
      ...
      /*
    IntegPolSIDIS::StrFunc xxs( args, IntegXsec::Charge::TARGET( k ),
				QCD::Flavor::PIp, 6, 8, 1.0E-2 );
    IntegPolSIDIS::PartialInteg xs( &xxs, 6, 8, 1.0E-2 );
    xs.setPIntegRange( 0.2, 0.8 );
      */
    IntegPolSIDIS::StrFunc xxs( args, IntegXsec::Charge::p, QCD::Flavor::PIp,
                                   4, 0, 1.0E-2, 10.0, 2.0, 0.55 );
// note default is 10.0, 1.8, 0.6. FF is so sensitive to angle ( 0.6 -> 0.55 )
    xxs.setGngrid( 4 ); 
    //    xxs.setloginteg( true ); 
    IntegPolSIDIS::PartialInteg xs( &xxs, 4, 0, 1.0E-2 );
    xs.setGngrid( 4 );
    xs.setPIntegRange( 0.2, 0.8 );
