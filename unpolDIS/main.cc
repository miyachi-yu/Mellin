...
int main( int argc, char* argv[] ){
  ...
  try{
    ...
    unpolDIS::CoeffFunc* kernel_;
    Xsection::Charge tar_( kernel_ );
/*
...
*/
    ...
    unpolDIS::StrFunc xs( args, Xsection::Charge::TARGET( k ) );
    dynamic_cast< Xsection::CKernel* >( xs.xsecComp()->coeff() )
      ->constNf( false );
...
//    for( int i = 0; i < vevo_.size(); i++ ) vevo_[i]->constNf( false );
    ...
    Xsection::xXsec xxc( &xs, 4, 6, 1.0E-4, 20.0, 1.8, 0.5 );//NLO
