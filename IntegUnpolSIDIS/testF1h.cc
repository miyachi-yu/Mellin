...
int main( int argc, char* argv[] ) {
  ...
  try {
    ...
    IntegUnpolSIDIS::StrFunc xf1hh( args, IntegXsec::Charge::p, 
				    ...
				    10.0, 2.0, 0.55 );
    xf1hh.setGngrid( ( args.hasOpt( "xShortInteg" ) ? 12 : 2 ) ); 
    if( args.hasOpt( "logInteg" ) )  xf1hh.setloginteg( true ); 

    IntegUnpolSIDIS::PartialInteg 
      xf1h( dynamic_cast< Transform::RealFunction* >( &xf1hh ), 
	    4, 0, 4, 1.0E-2 );
    xf1h.setPIntegRange( 0.2, 0.8 );
    // int dz / z * ( 2 x * z * f1h )
	...
	for( int iq2 = 0; iq2 < q2.size(); iq2++ ){
	  ...
	  
	  {
	    ...
	    for( int i=0; i < x.size(); i++ ){
	      xx_[ 0 ] = x[ i ]; 
	      cout << setw(13) << xx_[ 0 ] * f1hxx( xx_, zz_ ) 
	...
	for( int iq2 = 0; iq2 < q2.size(); iq2++ ){
	  ...
	  
	  {
	    ...
	    timer.start();
	    for( int i = 0; i < x.size(); i++ ){
	      cout << setw( 13 ) <<  xf1h( x[ i ] ) / 2.0 << flush;
	    }
	    timer.stop();
	    cout << setw( 13 ) << timer.check( Timer::Sec ) << " sec" << flush;
...

	{
	  ...
	  timer.start();
	  for( int i = 0; i < x.size(); i++ ){
	    dynamic_cast< IntegUnpolSIDIS::PStrFunc* >
	      ( f1hn.xsecComp()->coeff() )->setx_zPI( x[ i ] );
	    cout << setw( 13 ) 
		 <<  x[ i ] * f1hxz( unitv, unitv ) / 2.0 
		 << flush;
	  }
	  timer.stop();
	  cout << setw( 13 ) << timer.check( Timer::Sec ) << " sec" << flush;
