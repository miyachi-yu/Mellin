  ...
  Xsection::XsecComp()
{
  ...
  try {
    ...
    // prepare 1st Evolution kernel for polDrellYan function ( n )
    this->insert( polPDF::Evo::instance(), Flavor::PRp );
    dynamic_cast< polPDF::Evo* >( this->vevo()[ 0 ] )->constNf( false );
    
    // prepare 2nd Evolution kernel for polDrellYan function ( m )
...
void nXsecComp::coefficients( const IntegXsec::Charge::TARGET& n ){
  ...
  if( ! args.hasOpt( "PInteg" ) ){
    if( args.hasOpt( "ShortInteg" ) ){ 
      coeff = new IntegPolDrellYan::StrFunc( this, args, n, 4, 0, 1.0E-2 );
    } else {
      coeff = new IntegPolDrellYan::StrFunc( this, args, n, 4, 6, 1.0E-4 );
    }
  } else {
    /*
    if( args.hasOpt( "ShortInteg" ) ){ 
      coeff = new IntegPolDrellYan::PStrFunc( this, args, n, 4, 0, 1.0E-2 );
    } else {
      coeff = new IntegPolDrellYan::PStrFunc( this, args, n, 4, 6, 1.0E-4 );
    }
    */
    throw( 1 );
  }
